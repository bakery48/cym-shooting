...
export function recordRun(progress, stageId, cleared, stats) {
  ...
  const best = progress.best[stageId] ?? {};
  const kills = stats.ship + stats.pod;
  // 0 は「最高記録」として出さない（初回に必ず光ってしまい意味が薄れるため）
  if (stats.earned > (best.earned ?? 0)) { best.earned = stats.earned; updated.push('earned'); }
  if (kills > (best.kills ?? 0)) { best.kills = kills; updated.push('kills'); }
