...
**所持金と強化は区をまたいで永続する** ― 失敗しても、途中で抜けても戻らない。

持ち込めるのは**その面で買える強化だけ**（`carry`、既定では `shop` と同じ）。
「手動限定」にポッドを持ち込めてしまうとモードの意味が消えるため。
持ち込まなかった強化は財布に残ったままで、面を抜けても失われない。

面選択画面に持ち越しの所持金と強化が出る。記録はそこから消せる。
